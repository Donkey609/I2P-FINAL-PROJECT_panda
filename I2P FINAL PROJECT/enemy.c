...
void initEnemy(void){
    ...
    char* monsterPath = "Assets/monster.png";
    monsterBitmap = al_load_bitmap(monsterPath);
    if (!monsterBitmap) {
        game_abort("Error Load Bitmap with path : %s", monsterPath);
    }
}
...
bool updateEnemy(Enemy * enemy, Map * map, Player * player){
        ...
        */ 
        ...
        if (enemy->type == slime)
        {
            ...
            if (enemy->death_animation_tick == 63) {
                ...
                return true; //enemy deleted, biar gaada animasi lagi
            }
        }
        if (enemy-> type == monster)
        {
            enemy->death_animation_tick = (enemy->death_animation_tick + 1) % 25;
            if (enemy->death_animation_tick == 24) {
                enemy->death_animation_tick = 23;
                return true; //enemy deleted, biar gaada animasi lagi
            }
        }
        
    }//SAMPE SINI OKE SLIME ANIMATIONYA
    ...
    if (enemy->type == monster) {
        enemy->animation_tick = (enemy->animation_tick + 1) % 25;
    }
    ...
    if (distance < ATTACK_DISTANCE) {
        if (enemy->knockback_CD > 0) {
            ...
            */
            //Point delta = (Point){ 0, 0 }; // Enemy doesn't move
            ...
            //ADA YG SALAH JUGA DISINI
            if (enemy->type == monster) {
                if (playerCollision(enemy->coord, player->coord) && enemy->animation_hit_tick == 0) {
                    enemy->animation_tick = 0;
                    enemy->animation_hit_tick = 32;
                    stunPlayer(player, enemy->coord, 5);
                }
            }
        }
        return false; 
...
void drawEnemy(Enemy * enemy, Point cam){
    ...
    if(enemy->status == ALIVE){
        if (enemy->type == slime) {
            ...
            int offset = 50 * (int)(enemy->animation_tick / 5);
            int offsetY = 0;
            if (enemy->animation_hit_tick > 0) {
                offset = 50 * (int)((32 - enemy->animation_hit_tick) / (32/4));
                offsetY = 50;
            }
            int flag = enemy->dir == RIGHT ? 1 : 0;
            int tint_red = enemy->knockback_CD > 0 ? 255 : 0;
            al_draw_tinted_scaled_rotated_bitmap_region(enemy->image, offset, offsetY, 50, 50, al_map_rgb(tint_red, 255, 255),
                10, 10, dx, dy, TILE_SIZE / 25, TILE_SIZE / 25,
                0, flag);
        }
    }
        ...
        */
        //INI AKU TAMBAHIN
        if (enemy->type == slime) {
            int frame = enemy->death_animation_tick / 8; //8 soalnya di second row or death animation row cuman ad 8
            if (frame < 8) { //makesure framenya dalem limit
                int offset = 16 * frame;
                al_draw_tinted_scaled_rotated_bitmap_region(
                    enemy->image, offset, 16, 16, 16, al_map_rgb(255, 255, 255),
                    0, 0, dx, dy, TILE_SIZE / 16, TILE_SIZE / 16,
                    0, 0); // copas yg atas aj 
            }
        }
        else if (enemy->type == monster) {
            int frame = enemy->death_animation_tick / 5; //8 soalnya di second row or death animation row cuman ad 8
            if (frame < 5) { //makesure framenya dalem limit
                int offset = 50 * frame;

                al_draw_tinted_scaled_rotated_bitmap_region(
                    enemy->image, offset, 100, 50, 50, al_map_rgb(255, 255, 255),
                    10, 10, dx, dy, TILE_SIZE / 25, TILE_SIZE / 25,
                    0, 0); // copas yg atas aj 
            }
        }
...
static bool isCollision(Point enemyCoord, Map* map){
    ...
    /* 
        ...
        if(!isWalkable(map->map[...][...])) return true;
    */
    int eleft = enemyCoord.x / TILE_SIZE;
    int eright = (enemyCoord.x + TILE_SIZE - 1) / TILE_SIZE;
    int etop = enemyCoord.y / TILE_SIZE;
    int ebottom = (enemyCoord.y + TILE_SIZE - 1) / TILE_SIZE;
    if (!isWalkable(map->map[etop][eleft])) return true;
    if (!isWalkable(map->map[etop][eright])) return true;
    if (!isWalkable(map->map[ebottom][eleft])) return true;
    if (!isWalkable(map->map[ebottom][eright])) return true;
