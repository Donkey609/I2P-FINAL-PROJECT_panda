...
bool update_bullet(Bullet * bullet, enemyNode * enemyList, Map * map){
    ...
    */
    int tile_y = (int)(bullet->coord.x / TILE_SIZE);
    int tile_x = (int)(bullet->coord.y / TILE_SIZE);
    if (tile_x < 0 ||tile_y < 0 || tile_x >= map->row || tile_y >= map->col || map->map[tile_x][tile_y] == WALL) { //????
        return true;
    }
    ...
    while(cur != NULL){
        ...
        */
        if (bullet->coord.x >= enemyCoord.x && bullet->coord.x <= enemyCoord.x + TILE_SIZE &&
            ...
            // Bullet hits the enemy, call hitEnemy() function
            hitEnemy(&cur->enemy, bullet->damage, bullet->angle);
            return true;  // Bullet should be removed after hitting the enemy
        }
...
void updateBulletList(BulletNode * dummyhead, enemyNode * enemyList, Map * map){
    ...
    while (cur != NULL) {
        ...
        if (shouldDelete) {
            prev->next = cur->next; // Remove the bullet from the list
            destroy_bullet(&cur->bullet); // Free the bullet's resources
            free(cur); // Free the node
            cur = prev->next; // Move to the next bullet
        }
        else {
